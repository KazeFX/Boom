...
public class SimpleGame extends BasicGame{
	...
	throws SlickException {
		...
		cannon = 5;
		level = 1;
		laserImg = new Image("assets/laser.png");
	...
	throws SlickException {
		...
		for (Projectile p: ultiList){
			...
			/**
			ArrayList<Enemy> target = new ArrayList<Enemy>();
			for (Enemy e: EnemyList){
				float enemyY = e.getY();
				float enemyX = e.getX();
			
				
				if (enemyY < p.getY() && enemyX > p.getX()){
		...
		if (weaponMode.equals("V")){
			if (input.isKeyDown(Input.KEY_SPACE)){
				Projectile shot = new Projectile(system);
				shot.setPosition(plane.getX()+ 18, plane.getY()-100);
				shot.setImage(new Image("assets/laser.png"));
				laserList.add(shot);
			}
